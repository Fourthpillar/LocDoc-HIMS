...
public class DoctorService {
    ...
    public DoctorResponse verify(Long id) {
        ...
        // The source branch also writes a platform audit entry here (DOCTOR_VERIFY /
        // DOCTOR_REJECT). There is no audit-log module in this build yet; when one
        // arrives, verification and rejection are two of the events it must carry.
        log.info("Doctor {} ({}) verified by user {}", doctor.getId(), doctor.getRegistrationNumber(), SecurityUtils.currentUserId());
    ...
    public DoctorResponse reject(Long id) {
        ...
        log.info("Doctor {} ({}) rejected by user {}", doctor.getId(), doctor.getRegistrationNumber(), SecurityUtils.currentUserId());
        return response;
    }
